...
namespace NGFMReference
{
    public abstract class FinancialTermExtractor
    {
        ...
        public abstract bool GetTermsForGraph(Graph graph, out String error);

        protected abstract void SetContractSubject();

        public delegate void UpdateGraphDelegate(Dictionary<string, object> termDictionary, Graph graph);

        protected bool GetContractComponentForGraph(Graph graph, Dictionary<string, object> jsonParseResult, string ComponentName, out string message)
        {
            message = "";
            object Component;
        ...
        private void UpdateGraphWithDed(Dictionary<string, object> termDictionary, Graph graph)
        {
        ...
        private void UpdateGraphWithCover(Dictionary<string, object> coverDictionary, Graph graph) 
        {
            bool nodefound = false;
    ...
    public class PrimaryTermExtractor : FinancialTermExtractor
    {
        ...
        public override bool GetTermsForGraph(Graph graph, out String error)
        {
        ...
        private bool SetDeclarations(Declarations graph_declarations, Dictionary<String, Object> cdl_declarations, out string message)
        {
            ...
            try
            {
                ...
                if (cdl_declarations.ContainsKey("Schedule"))
                    graph_declarations.Schedule = expData.GetSchedule(Convert.ToString(cdl_declarations["Schedule"]));

                if (cdl_declarations.ContainsKey("GrossPosition"))
                    graph_declarations.GrossPosition = Convert.ToString(cdl_declarations["GrossPosition"]);
            ...
            {
                message = "Error in setting declarations :" + ex.Message;
                return false;
            }
            return true;
        }
