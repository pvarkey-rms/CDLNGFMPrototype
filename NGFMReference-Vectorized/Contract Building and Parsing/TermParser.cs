...
namespace NGFMReference
{
    public static class TermParser
    {
        ...
        public static Cover GetCoverForTerm(Dictionary<string, object> coverDictionary)
        {
            ...
            bool unlimitedReinstatements = false;
            ...
            int reinstatementsFromCDL;
            if (coverDictionary.ContainsKey("LimitReinstatements"))
                reinstatementsFromCDL = (int)Enum.Parse(typeof(TimeBasis), coverDictionary["LimitReinstatements"].ToString());
            else
                reinstatementsFromCDL = -1;

            if (reinstatementsFromCDL >= 0)
                NumOfReinstatements = (uint)reinstatementsFromCDL;
            else
                unlimitedReinstatements = true;

            return new Cover(name, franchise, attpoint, limit, (PercentValue)proRata, unlimited, attTimeBasis, limitTimeBasis, vType, NumOfReinstatements, unlimitedReinstatements);
        }
        ...
        private static void GetInfoFromJSONExp(Dictionary<string, object> expDict, out Value result, out TermValueType termValType)
        {
            ...
            if (parsedValue is FunctionValue)
            {
                ...
                else if (function.Function == FunctionType.WaitingPeriod)
                {
                    ...
                    else throw new NotSupportedException("Cannot handle BI waiting period that is not specified as days...");
                } 
                else
                {                                        
        ...
        private static Value RecursiveParseJSONValue(Dictionary<string, object> valueDict)
        {
            ...
            if (valueDict["ExpressionType"].ToString() == "SimpleExpression<NumericValue>" || valueDict["ExpressionType"].ToString() == "NumericExpression" || valueDict["ExpressionType"].ToString() == "Money<NumericValue>")
            {
                if (valueDict.ContainsKey("PAY") && valueDict["PAY"].ToString() == "True")
                {
                    FunctionValue value = new FunctionValue(FunctionType.Constant, PasreSimpleExpression(valueDict));
                    return value;
                }
                else
                        ...
                        if (valueDict["FunctionParameterValueType"].ToString() == "IExpression<Value>")
                        {
                            ...
                            if (valueDict["ValueType"].ToString() == "Value")
                            {
                                ...
                                foreach (object argumentObject in argumentObjects)
                                {
                                    ...
                                    {
                                        function.AddArgument(new SymbolicValue(argumentValueDict["Value"].ToString()));
                                    }
